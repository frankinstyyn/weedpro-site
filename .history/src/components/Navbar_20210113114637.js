...
export default class Navbar extends Component {
  ...
  componentDidMount() {
    window.addEventListener("scroll", this.TransparentHeaderOnScroll);
  }
  TransparentHeaderOnScroll() {
    const distanceY = window.pageYOffset || document.documentElement.scrollTop,
      shrinkOn = 100,
      navT = document.getElementByClassName("navbar");

    if (distanceY > shrinkOn) {
      navT.classList.add("navbarBackground");
    } else {
      navT.classList.remove("navbarBackground");
    }
  }
