...
export default class Navbar extends Component {
  ...
  render() {
    return <nav id="navbar">
      <div className="nav-center" style={centerWidth}>
        <div className="nav-header">
          <Link to="/">
            <img id="logo" src={logo} alt="Weed Pro UK"/>
          </Link>
          <button type="button" className="nav-btn" onClick={this.handleToggle} duration={500}>
            <FaAlignRight className="nav-icon" />
          </button>
        </div>
        <ul className={this.state.isOpen ? "nav-links show-nav" : "nav-links align-middle"}>
          <li>
            <Link to="/" onClick={this.handleToggle}>Home</Link>
          </li>
          <li>
            <Link to="/cleaning-services" onClick={this.handleToggle}>Services</Link>
          </li>
          <li>
            <Link to="/contact" onClick={this.handleToggle}>About</Link>
          </li>
          </ul>
      </div>
    </nav>;
